...
function assertValidTemplateRoot(root: string): void {
  if (!fs.existsSync(path.join(root, 'base'))) {
    throw new Error(
      `模板源 "${root}" 不符合约定：缺少 base/ 目录。请确保模板源包含 base/、entry/、features/ 目录结构。`,
    );
  }
}
...
export function inspectTemplateSource(templateRoot: string): TemplateInspection {
  ...
  const entries = fs
    ...
    .filter(
      (entry) =>
        entry.isDirectory() && fs.existsSync(path.join(templateRoot, entry.name, 'base')),
    )
...
export async function selectTemplateEntry(
  ...
  if (prefName) {
    ...
    if (!matched) {
      throw new Error(
        `未找到模板 "${prefName}"，可用模板：${entries.map((e) => e.name).join(', ')}`,
      );
    }
    return matched.root;
  }
...
export function loadTemplatePresets(templateRoot: string): QuestionPresets {
  ...
  try {
    ...
    return {
      ...
      css:
        raw.css === 'scss' || raw.css === 'less' || raw.css === 'stylus' || raw.css === 'none'
          ? raw.css
          : undefined,
      state:
        raw.state === 'pinia' ||
        raw.state === 'pinia-persist' ||
        raw.state === 'vuex' ||
        raw.state === 'vuex-persist' ||
        raw.state === 'none'
          ? raw.state
          : undefined,
      router:
      ...
      theme:
        ...
        (raw.theme === 'none' || (THEME_IDS as readonly string[]).includes(raw.theme))
          ? (raw.theme as QuestionPresets['theme'])
          : undefined,
      platform: Array.isArray(raw.platform) && raw.platform.every((p) => typeof p === 'string')
        ? (raw.platform as QuestionPresets['platform'])
        : undefined,
      features: Array.isArray(raw.features) && raw.features.every((f) => typeof f === 'string')
        ? (raw.features as QuestionPresets['features'])
        : undefined,
      packageManager:
        raw.packageManager === 'npm' || raw.packageManager === 'yarn' || raw.packageManager === 'pnpm'
          ? raw.packageManager
          : undefined,
