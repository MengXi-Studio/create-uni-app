...
import { copyDir, getTemplatesRoot, renderTemplate } from './utils';
...
function buildContext(options: CreateOptions): TemplateContext {
  ...
  return {
    ...
    themeImport:
      options.css === 'none' && options.theme !== 'none'
        ? "import '@/theme.css'"
        : '',
    uniVersion,
    ...
    paletteLines:
      options.theme !== 'none'
        ? renderThemeVars(options.theme as Theme, options.css)
        : '',
...
function buildCssPreprocessorConfig(options: CreateOptions): string {
  ...
  return [
    ...
    '    preprocessorOptions: {',
    `      ${lang}: {`,
    `        additionalData: \`@import "@/${themeFile}";\`,`,
    '      },',
    '    },',
    '  },',
  ].join('\n');
...
function buildPlatformManifestBlocks(options: CreateOptions): string {
  ...
  const blocks: string[] = [];
  // 小程序平台（除微信外均用最简配置）
  const simpleMp: Exclude<TargetPlatform, 'multi'>[] = [
    'mp-alipay',
    'mp-baidu',
    'mp-toutiao',
    'mp-qq',
