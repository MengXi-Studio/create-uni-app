...
| 命令 | 作用 |
| --- | --- |
| `create-uni-app [projectName]` | 创建项目（顶层） |
| `create-uni-app create` | 在项目中新建页面 |
| `create-uni-app info` | 打印环境与项目诊断信息 |
| `create-uni-app config` | 查看 / 设置模板源配置 |
...
> 模板源解析优先级：`--template-source` 参数 > 全局配置 > 内置模板。详见 [配置持久化](./config)。
...
自动探测项目形态并正确注册路由，详见 [创建页面](./create-page)。
...
打印 CLI / Node 版本，并检测当前目录是否为 uni-app 项目及是否启用 TS / Sass / Pinia / Vuex / uni-ui。在提 issue 时附上 `info` 输出，便于定位问题。
